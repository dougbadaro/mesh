...
export function ExportCsvCard() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const suffix =
        selectedMonth === "all" ? "completo" : `${Number(selectedMonth) + 1}-${selectedYear}`
    ...
    <Card className="overflow-hidden rounded-3xl border-white/5 bg-zinc-900/40 shadow-sm backdrop-blur-xl">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <CardDescription className="text-[10px] text-zinc-500">
              Selecione o período e baixe seus dados.
            </CardDescription>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-5 p-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1.5">
            ...
            <Select value={selectedMonth} onValueChange={setSelectedMonth}>
              <SelectTrigger className="h-9 rounded-xl border-white/10 bg-zinc-950/50 text-sm focus:ring-emerald-500/30">
                <SelectValue placeholder="Mês" />
              </SelectTrigger>
              <SelectContent className="max-h-[200px] border-white/10 bg-zinc-900">
                {months.map((m) => (
        ...
        <Button
          ...
          disabled={isLoading}
          className="h-9 w-full rounded-xl bg-white text-xs font-bold text-black shadow-lg shadow-white/5 transition-transform hover:bg-zinc-200 active:scale-[0.98]"
        >
          {isLoading ? (
            <>
