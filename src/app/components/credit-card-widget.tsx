...
import { CreditCard, Calendar, Receipt, ChevronRight } from "lucide-react"
    ...
    <Card className="bg-zinc-900/60 backdrop-blur-md border-white/5 shadow-xl relative overflow-hidden h-fit w-full">
      ...
      <CardContent className="relative z-10 pt-4 px-6 pb-6 space-y-4">
        {invoices.length === 0 ? (
          ...
          invoices.map((inv) => (
            <div key={inv.id} className="group bg-zinc-950/50 border border-white/5 hover:border-white/10 transition-all rounded-xl overflow-hidden">
              ...
              <div className="p-4 flex items-center justify-between bg-gradient-to-r from-zinc-900/50 to-transparent border-b border-white/5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold text-zinc-200 capitalize tracking-wide leading-none">{inv.monthLabel}</p>
                    <p className="text-[10px] text-zinc-500 uppercase tracking-wider font-bold mt-1">Vencimento</p>
                  </div>
                </div>
                <div className="text-right">
                  <span className="block text-rose-400 font-bold tabular-nums text-lg leading-none shadow-rose-500/10 drop-shadow-sm">
